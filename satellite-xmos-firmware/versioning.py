...
TO_TRACK = ["{variant}.factory.bin", "{variant}.factory.md5", "{variant}.xe"]

PRE_RELEASES = ["none", "alpha", "beta", "rc", "dev"]
VERSION_REGEX = re.compile(
    r"^v?(?P<major>\d+)\.(?P<minor>\d+)\.(?P<patch>\d+)([-.]?(?P<prerelease>[a-zA-Z]+)(\.(?P<counter>\d+))?)?$"
)
...
class XMOSVersion:
    ...
    def from_string(cls, vstr: str) -> "XMOSVersion":
        ...
        return cls(
            ...
            pre_release=PRE_RELEASES.index(groups["prerelease"])
...
class GitInfo:
    ...
    def __str__(self):
        return (
            f"{self.commit}" + ("+patch" if self.patch_str else "") + f"@{self.branch}"
        )
...
class TrackedDevBuild:
    ...
    def __eq__(self, other):
        return (
            ...
            and self.git_info == other.git_info
            and self.patch_file_md5 == other.patch_file_md5
        )
    ...
    @classmethod
    def latest(
        cls, dev_track_path: Path, base_version: Union[XMOSVersion, None] = None
    ) -> "TrackedDevBuild":
            ...
            [
                ...
                if (
                    ...
                    and (
                        ...
                        or f.name.startswith(base_version.base_version_string)
                    )
                )
            ],
            reverse=True,
...
def create_patch_file(repo_root: Path, patch_file: Path) -> str:
    try:
        with patch_file.open("w") as f:
            subprocess.run(["git", "diff"], cwd=repo_root, stdout=f)
    except:
...
def track_dev_build(args: argparse.Namespace) -> TrackedDevBuild:
    ...
    last_dev_build = TrackedDevBuild.latest(dev_track_path, last_tag_version)
...
def print_info(args: argparse.Namespace) -> None:
    ...
    print(
        f"{VERSION_HEADER_FILE.relative_to(Path.cwd())}: {'not found' if from_header_file is None else str(from_header_file)}"
    )
    if tracked_build:
        ws_build_xe = args.build_dir / (tracked_build.variant + ".factory.bin")
        tracked_build_xe = tracked_build.track_path / (
            tracked_build.variant + ".factory.bin"
        )
        same_build = (
            ws_build_xe.exists()
            and tracked_build_xe.exists()
            and filecmp.cmp(tracked_build_xe, ws_build_xe)
        )
        print(
            textwrap.dedent(f"""\
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Manages XMOS firmware versions.",
    ...
    info_parser = subparsers.add_parser(
        "info", help="Show workspace version information"
    )
    info_parser.set_defaults(func=print_info)

    build_parser = subparsers.add_parser(
        "build", help="Pass XMOS firmware version to build system."
    )
    ...
    install_parser = subparsers.add_parser(
        "track", help="Copy variant targets to tracked build folder."
    )
    ...
    if args.version is None and args.infile is None:
        if VERSION_FILE.exists():
            ...
            parser.error(
                "{VERSION_FILE} not found. Use --version or --infile, or ensure 'firmware_version.txt' exists."
            )

    if args.command == "track":
        args.track = True

    if isinstance(args.build_dir, list):
        args.build_dir = args.build_dir[0]
    if isinstance(args.track_root, list):
        args.track_root = args.track_root[0]
    if not isinstance(args.build_dir, Path):
        args.build_dir = Path(args.build_dir)
    if not isinstance(args.track_root, Path):
        args.track_root = Path(args.track_root)
