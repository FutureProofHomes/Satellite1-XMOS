...
foreach(FFVA_AP ${FFVA_PIPELINES_INT})
    ...
    target_link_libraries(${TARGET_NAME} PUBLIC ${APP_COMMON_LINK_LIBRARIES} fph::ffva::sq66 fph::ffva::ap::${PL_NAME} sln_voice::app::ffva::sp::passthrough ${CMAKE_CURRENT_LIST_DIR}/src/config.xscope)
    ...
    add_custom_target(sq66_firmware_${FFVA_AP}_versioning
        COMMAND ${Python3_EXECUTABLE} ${VERSIONING_SCRIPT} ${VERSIONING_CMD} sq66_firmware_${FFVA_AP}
        COMMENT "Running versioning.py build sq66_firmware_${FFVA_AP}"
        VERBATIM
    )
    add_dependencies(tile0_sq66_firmware_${FFVA_AP} sq66_firmware_${FFVA_AP}_versioning)
    add_dependencies(tile1_sq66_firmware_${FFVA_AP} sq66_firmware_${FFVA_AP}_versioning)
    ...
    set(TARGET_NAME sq66_firmware_${FFVA_AP})
    ...
    add_custom_target(${FATFS_FILE} ALL
    ...
    set_target_properties(${FATFS_FILE} PROPERTIES ADDITIONAL_CLEAN_FILES ${FATFS_CONTENTS_DIR})
    add_custom_command(OUTPUT ${DATA_PARTITION_FILE}
        COMMAND ${CMAKE_COMMAND} -E copy ${FATFS_FILE} ${DATA_PARTITION_FILE}
        DEPENDS ${FATFS_FILE}
        COMMENT "Create data partition"
        VERBATIM
    )
    list(APPEND DATA_PARTITION_FILE_LIST ${FATFS_FILE} ${DATA_PARTITION_FILE})
    create_data_partition_directory(${TARGET_NAME} "${DATA_PARTITION_FILE_LIST}" "${DATA_PARTITION_FILE_LIST}")
    create_flash_image_target(${TARGET_NAME} 0x100000)
    create_flash_app_target(${TARGET_NAME} 0x100000 ${DATA_PARTITION_FILE} ${DATA_PARTITION_FILE})
