...
def _run_remote_sdk_json(
    ...
    for attempt in range(RETRY_ATTEMPTS):
        last = _run_remote_sdk_python(host, sat1_rpi_py_cmd, script, timeout=timeout)
...
def _get_audio_settings(sat1_rpi_host: str, sat1_rpi_py_cmd: str) -> dict:
    script = """
import json
...
def audio_settings_guard(
    ...
    original = _get_audio_settings(sat1_rpi_host, sat1_rpi_py_cmd)
    yield original
...
def test_mic_input_gain_roundtrip_sat1(
    ...
    current = _get_audio_settings(sat1_rpi_host, sat1_rpi_py_cmd)
...
def test_mic_gain_changes_captured_level_sat1(
    ...
    _set_mic_output_packing(sat1_rpi_host, sat1_rpi_py_cmd, enabled=False)
    _set_mic_output_channels(sat1_rpi_host, sat1_rpi_py_cmd, 4, 4)
    ...
    time.sleep(0.3)
    low_rms = _capture_rms(sat1_rpi_host, duration_s=2)
    ...
    time.sleep(0.3)
    high_rms = _capture_rms(sat1_rpi_host, duration_s=2)
...
def test_ref_gain_changes_captured_level_sat1(
    ...
    _set_mic_output_packing(sat1_rpi_host, sat1_rpi_py_cmd, enabled=False)
    _set_mic_output_channels(sat1_rpi_host, sat1_rpi_py_cmd, 0, 0)

    tone_proc = _start_remote_tone(sat1_rpi_host)
    try:
        time.sleep(1.0)
