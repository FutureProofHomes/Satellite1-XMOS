...
for ((i = 0; i < ${#examples[@]}; i += 1)); do
    ...
    (cd ${path}/build_${board}; log_errors md5sum ${app_target}.factory.bin | cut -f 1 -d " " > ${app_target}.factory.md5 )
    ...
    (cd ${path}/build_${board}; log_errors md5sum ${app_target}.upgrade.bin | cut -f 1 -d " " > > ${app_target}.upgrade.md5 )
