...
help()
{
   echo "XCORE firmware builder script"
   echo
   echo "Syntax: build_firmware.sh [-h] <firmware_list.txt>"
   echo
   echo "firmware_list.txt format:"
...
for ((i = 0; i < ${#examples[@]}; i += 1)); do
    ...
    #(cd ${path}/build_${board}; log_errors md5 -q ${app_target}.factory.bin > ${app_target}.factory.md5 )
    (cd ${path}/build_${board}; cp ${app_target}.factory.bin ${DIST_DIR})
    (cd ${path}/build_${board}; log_errors $CI_BUILD_TOOL create_upgrade_img_${app_target} $CI_BUILD_TOOL_ARGS)
    #(cd ${path}/build_${board}; log_errors md5 -q ${app_target}.upgrade.bin > ${app_target}.upgrade.md5 )
    (cd ${path}/build_${board}; cp ${app_target}.upgrade.bin ${DIST_DIR})
